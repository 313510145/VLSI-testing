...
void CIRCUIT::FaultCPSimVectors() {
    cout << "Run stuck-at fault simulation with checkpoint theorem" << endl;
    unsigned pattern_num(0);
    if (!Pattern.eof()) {
        while (!Pattern.eof()) {
            ++pattern_num;
    ...
    FAULT* fptr;
    list<FAULT*>::iterator fite;
    for (fite = FlistCP.begin(); fite != FlistCP.end(); ++fite) {
        fptr = *fite;
        switch (fptr->GetStatus()) {
            case DETECTED:
                ++eqv_detected_num;
    ...
    cout << "Undetected fault number = " << undetected_num << endl;
    cout << "---------------------------------------" << endl;
    cout << "Equivalent fault number = " << FlistCP.size() << endl;
    cout << "Equivalent detected fault number = " << eqv_detected_num << endl; 
    cout << "Equivalent undetected fault number = " << eqv_undetected_num << endl; 
    cout << "---------------------------------------" << endl;
    cout << "Fault Coverge = " << 100 * detected_num / double(total_num) << "%" << endl;
    cout << "Equivalent FC = " << 100 * eqv_detected_num / double(FlistCP.size()) << "%" << endl;
    cout << "---------------------------------------" << endl;
    return;
}
...
void CIRCUIT::FaultCPSim() {
    ...
    list<FAULT*>::iterator fite;
    for (fite = UFlistCP.begin(); fite != UFlistCP.end(); ++fite) {
        fptr = *fite;
    ...
    for (fite = UFlistCP.begin(); fite != UFlistCP.end();) {
        fptr = *fite;
        if (fptr->GetStatus() == DETECTED || fptr->GetStatus() == REDUNDANT) {
            fite = UFlistCP.erase(fite);
        }
        else {
            ++fite;
        }
